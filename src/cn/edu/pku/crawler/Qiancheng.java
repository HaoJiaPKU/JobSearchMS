...
public class Qiancheng {
	...
	public static void proData() {
		...
		File file = new File("../../myFile/51job 训练集_pro");
	...
	public void parseIndexPage(String content, String saveDir) {
		...
		for (Element table : tables) {
			...
			crawlPositionPage(link, saveDir + "/" + value);
		}
	}
	...
	public void crawlPositionPageBatch(QianchengConf qc) {
		for (int i = 0; i < qc.getIndustryDir().size(); i ++) {
			...
			for (int j = 1; j <= qc.getMaxPageNumber(); j ++) {
				String content = getIndexPage(qc.getIndustryUrl().get(i) + String.valueOf(j));
